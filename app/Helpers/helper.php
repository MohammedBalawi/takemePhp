...
function getSettingFirstData($type = null, $key = null)
{
    ...
    try {
        $value = app(FirestoreRestService::class)->getSetting($type ?? '', $key ?? '');
        if ($value !== null) {
            return ['image' => $value];
        }
    } catch (\Throwable $e) {
...
function getSingleMediaSettingImage($collection_name, $model = null, $check_collection_type = null)
{
    $image = null;
    if ($model !== null) {
        if (is_array($model) && isset($model['image']) && is_string($model['image']) && $model['image'] !== '') {
            return $model['image'];
        }
        if (is_object($model)) {
            if (isset($model->image) && is_string($model->image) && $model->image !== '') {
                return $model->image;
            }
            if (isset($model->image_url) && is_string($model->image_url) && $model->image_url !== '') {
...
function authSession($force = false) {
    ...
    if (!is_array($admin)) {
        return (object) [
            ...
            'hasAnyRole' => function () { return false; },
        ];
    }
...
function appSettingData($type = 'get')
{
    ...
    if ($type === 'set' || $cache === null) {
        $data = app(AppData::class)->getAppSettings();
        if (!is_array($data)) {
            $data = [];
        }
        if (!isset($data['language_option'])) {
            $data['language_option'] = 'ar';
        }
        $cache = (object) $data;
        Session::put('setting_data', $cache);
    }
...
{
    return (object) app(AppData::class)->getAppSettings();
}
...
function saveRideHistory($data)
{
            ...
            if ($email) {
                ...
                if ($emailData) {
                    ...
                    $data = [
                        ...
                        'status' => $status_key,
                        'company_name' => AppSetting('company_name', 'Your Company'),
    ...
    switch ($data['history_type']) {
        ...
        case 'arriving':
            ...
            $sendEmail($ride_request, 'arriving', $dynamicData);
            // $mqtt_event = 'ride_request_status';
            $sendTo = removeValueFromArray(['admin', 'rider'], $user_type);
            break;
        ...
        case 'arrived':
            ...
            // $mqtt_event = 'ride_request_status';
            $sendEmail($ride_request, 'arrived', $dynamicData);
            $sendTo = removeValueFromArray(['admin', 'rider'], $user_type);
            break;
        ...
        case 'canceled':
            ...
            $sendEmail($ride_request, 'canceled', $dynamicData);
        ...
        case 'driver_canceled':
            ...
            // $mqtt_event = 'ride_request_status';
            $sendEmail($ride_request, 'driver_canceled', $dynamicData);
            $sendTo = removeValueFromArray(['admin', 'rider'], $user_type);
            break;
        ...
        case 'rider_canceled':
            ...
            $sendEmail($ride_request, 'rider_canceled', $dynamicData);
            $sendTo = removeValueFromArray(['admin', 'driver'], $user_type);
            break;
        ...
        case 'completed':
            ...
            // \Log::info("Email sent for ride ID: {$ride_request->id}, status: completed");
...
function checkMenuRoleAndPermission($menu)
{
    ...
    if ($menu->data('role') != null) {
        $neededRoles = array_map('trim', explode(',', $menu->data('role')));
...
function getSingleMedia($model, $collection = 'profile_image', $skip=true   )
{
    ...
    $media = null;
    if ($model !== null) {
        if (is_array($model) && isset($model[$collection]) && is_string($model[$collection]) && $model[$collection] !== '') {
            return $model[$collection];
        }
        if (is_object($model)) {
            if (isset($model->$collection) && is_string($model->$collection) && $model->$collection !== '') {
                return $model->$collection;
            }
            if (method_exists($model, 'getFirstMedia')) {
...
function mighty_get_distance_matrix($pick_lat, $pick_lng, $drop_lat, $drop_lng, $traffic = false) {
    ...
    $response = Http::withHeaders([
        ...
        ->connectTimeout(5)
        ->timeout(10)
        ->get('https://maps.googleapis.com/maps/api/distancematrix/json?origins='.$pick_lat.','.$pick_lng.'&destinations='.$drop_lat.','.$drop_lng.'&mode=driving');
...
function calculateRideFares($distance_in_unit, $pickupLat, $pickupLng, $dropLat, $dropLng, $multiLocation, $dropoff_time_in_seconds, $service, $coupon = null, $surge_price = null,$ride_datetime = null) {
    ...
    if (!is_array($multiLocation)) {
        \Log::error("Invalid multiLocation format", ['multiLocation' => $multiLocation]);
        $multiLocation = [];
    }

    $multiLocation = array_filter($multiLocation, function($location) {
        return is_array($location) && isset($location['lat']) && isset($location['lng']);
    });
    ...
    // Finally, add distance from the last drop location to the final destination
    $finalDistance = haversineDistance($previousLat, $previousLng, $dropLat, $dropLng);
    $distance_in_unit += $finalDistance;

    // Add distance from last point (or pickup point if no multi-location) to final drop-off point
    if ($distance_unit == 'mile') {
        $distance_in_unit = km_to_mile($distance_in_unit);
    }
...
function SettingData($type, $key = null)
{
    ...
    $firestoreValue = null;
    if ($key !== null && isFirestoreEnabled()) {
        if (array_key_exists($cacheKey, $firestoreCache)) {
            $firestoreValue = $firestoreCache[$cacheKey];
        } else {
            try {
                $firestoreValue = app(FirestoreRestService::class)->getSetting($type, $key);
                $firestoreCache[$cacheKey] = $firestoreValue;
            } catch (\Throwable $e) {
                $firestoreCache[$cacheKey] = null;
                $firestoreValue = null;
            }
        }

        if ($firestoreValue !== null) {
            $resolvedCache[$cacheKey] = $firestoreValue;
            return $firestoreValue;
        }

        if (!array_key_exists($cacheKey, $missingLogged)) {
    ...
    if ($type === 'app_info' && $key === 'app_title') {
        $resolvedCache[$cacheKey] = 'Take Me';
        return 'Take Me';
    }
...
{
    \Log::info("Drop Location Received", ['drop_latlng' => $drop_latlng]);

    // Ensure drop locations are an array
    ...
    if (!is_array($drop_latlng)) {
        $drop_latlng = [$drop_latlng];
    }

    // Filter valid locations
    $drop_latlng = array_filter($drop_latlng, function ($location) {
    ...
    for ($i = 0; $i <= $count; $i++) {
        if ($i == 0) {
            $fromLat = $pick_lat;
            $fromLng = $pick_lng;
            $toLat = $drop_latlng[$i]['lat'] ?? null;
            $toLng = $drop_latlng[$i]['lng'] ?? null;
        } elseif ($i == $count) {
            $fromLat = $drop_latlng[$i - 1]['lat'] ?? null;
            $fromLng = $drop_latlng[$i - 1]['lng'] ?? null;
            $toLat = $drop_lat;
            $toLng = $drop_lng;
        } else {
            $fromLat = $drop_latlng[$i - 1]['lat'] ?? null;
        ...
        if ($fromLat === null || $fromLng === null || $toLat === null || $toLng === null) {
            \Log::warning("Skipping invalid drop location", [
        ...
        $response = mighty_get_distance_matrix($fromLat, $fromLng, $toLat, $toLng);
    ...
    \Log::info("Total Distance & Duration", ['distance' => $distance, 'duration' => $duration]);

    return [
        'distance' => $distance ?? 0, // Ensure distance exists
        'duration' => $duration ?? 0  // Ensure duration exists
    ];
}
