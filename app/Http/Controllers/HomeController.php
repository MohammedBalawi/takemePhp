...
class HomeController extends Controller
{
    ...
    public function map()
    {
        ...
        $assets = ['map'];
        $markers = app(\App\Services\DriversMapService::class)->listOnlineDrivers();
        return view('map.index', compact('pageTitle','assets','markers'));
    }

    public function driverDetail(Request $request)
    {
        $data = app(\App\Services\DriversMapService::class)->getDriverProfile((string) $request->id);
        return response()->json(['data' => $data]);
    }
    public function search($id)
    {
        $data = app(\App\Services\DriversMapService::class)->getDriverProfile((string) $id);
        if (!empty($data)) {
            return response()->json(['data' => $data]);
        }
