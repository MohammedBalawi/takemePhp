...
class LDDFW_Driver {
	...
	public function set_order_commission__premium_only( $order, $order_id ) {
		...
		if ( '' === $order_commission ) {
			...
			if ( '' !== $lddfw_driver_commission_value && '' !== $lddfw_driver_commission_type ) {
				...
				if ( 'distance' === $lddfw_driver_commission_type ) {
						...
						if ( isset( $origin_distance['distance_text'] ) ) {
							...
							if ( 0 < $distance ) {
								...
								if ( 'imperial' === $unit_system ) {
									$commission = $distance / 1609 * $lddfw_driver_commission_value;
								} else {
									$commission = $distance / 1000 * $lddfw_driver_commission_value;
								}
								$commission      = round( $commission );
								$commission_note = $origin_distance['distance_text'] . ' ' . esc_html( __( 'from pickup address.', 'lddfw' ) );
							}
						}
					}
				}
	...
	public function get_vehicle_info__premium_only( $driver_id, $format ) {
		...
		if ( '' !== $lddfw_driver_vehicle || '' !== $lddfw_driver_licence_plate ) {
			if ( 'html' === $format ) {
				echo '<p><b>' . esc_html( __( 'Vehicle', 'lddfw' ) ) . ':</b><br>';
				echo esc_html( $lddfw_driver_vehicle ) . '<br>';
				echo esc_html( $lddfw_driver_licence_plate ) . '</p>';
			} else {
				echo esc_html( __( 'Vehicle', 'lddfw' ) ) . "\n";
				echo esc_html( $lddfw_driver_vehicle ) . "\n";
				echo esc_html( $lddfw_driver_licence_plate ) . "\n";
			}
		}
	}
	...
	public function get_driver_info__premium_only( $driver_id, $format, $permission = false ) {
		...
		if ( true === $permission || true === $photo_permission || true === $name_permission || true === $phone_permission ) {
			if ( 'html' === $format ) {
				echo '<h2 class="woocommerce-column__title"><b>' . esc_html( __( 'Driver details', 'lddfw' ) ) . '</b></h2>';
			} else {
				echo esc_html( __( 'Driver details', 'lddfw' ) ) . "\n";
			}
		}
		...
		if ( 'html' === $format ) {
			...
			if ( true === $permission || true === $photo_permission ) {
				...
				if ( intval( $image_id ) > 0 ) {
					$image = wp_get_attachment_image_src( $image_id, 'medium' )[0];
					if ( '' !== $image ) {
						echo '<img style="width:auto;max-width:100px" src="' . $image . '"><br>';
					}
				}
			}
			...
			if ( true === $permission || true === $phone_permission ) {
				...
				if ( '' !== $lddfw_driver_billing_phone ) {
					echo '<a href="tel:' . $lddfw_driver_billing_phone . '">' . __( 'Call driver', 'lddfw' ) . ': ' . $lddfw_driver_billing_phone . '</a>';
				}
			}
		} else {
