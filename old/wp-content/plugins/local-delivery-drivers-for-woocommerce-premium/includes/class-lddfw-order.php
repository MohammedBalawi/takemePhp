	...
	<div class="container" id="lddfw_order">
		...
		if ( lddfw_fs()->is__premium_only() ) {
			if ( lddfw_fs()->is_plan( 'premium', true ) ) {
					...
					$store->lddfw_store_name__premium_only( $order, $seller_id ) . ' <br> ' . $store->lddfw_pickup_address( 'address', $order, $seller_id );
				if ( '' !== $store_phone ) {
					$html .= '<br> ' . esc_html( __( 'Phone', 'lddfw' ) ) . ': ' . $store_phone . '<br>';
				}

					$html .= '<div class="row" style="margin-top:10px">';
				if ( '' !== $store_phone ) {
					$html .= '<div class="col"><a class="btn col btn-block btn-secondary"  href="tel:' . esc_attr( $store_phone ) . '"><svg aria-hidden="true" focusable="false" data-prefix="fas" data-icon="phone" class="svg-inline--fa fa-phone fa-w-16" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><path fill="currentColor" d="M493.4 24.6l-104-24c-11.3-2.6-22.9 3.3-27.5 13.9l-48 112c-4.2 9.8-1.4 21.3 6.9 28l60.6 49.6c-36 76.7-98.9 140.5-177.2 177.2l-49.6-60.6c-6.8-8.3-18.2-11.1-28-6.9l-112 48C3.9 366.5-2 378.1.6 389.4l24 104C27.1 504.2 36.7 512 48 512c256.1 0 464-207.5 464-464 0-11.2-7.7-20.9-18.6-23.4z"></path></svg> ' . esc_html( __( 'Call', 'lddfw' ) ) . '</a></div>';
				}
		...
		if ( lddfw_fs()->is__premium_only() ) {
			if ( lddfw_fs()->is_plan( 'premium', true ) ) {
				...
				$duration_style = '' !== $duration_text && '' !== $lddfw_order_delivery_start ? '' : 'style="display:none;"';
				$html          .= '<div id="driver_duration_section" class="col-12" ' . $duration_style . '>
									<p>'
									. esc_html( __( 'Estimated time of arrival', 'lddfw' ) ) . ': <span id="driver_duration">' . esc_html( $duration_text ) . '</span>
									</p>
								</div>';
			}
		}
