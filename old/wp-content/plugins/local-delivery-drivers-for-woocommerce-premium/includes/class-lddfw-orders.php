...
class LDDFW_Orders {
	...
	public function lddfw_out_for_delivery( $driver_id ) {
		...
		if ( ! empty ( $results ) ) {
			...
			foreach ( $results as $result ) {
				...
				// Print coordinates.
				if ( '' !== $coordinates ){
					$html .= '<a class="lddfw_order_address lddfw_order_coordinates" href="' . esc_url( lddfw_drivers_page_url( 'lddfw_screen=order&lddfw_orderid=' . $orderid ) ) . '">
				...
				if ( lddfw_fs()->is__premium_only() ) {
					if ( lddfw_fs()->is_plan( 'premium', true ) ) {
						...
						if ( '' !== $custom_fields ) {
								$html .= '<a class="lddfw_order_custom" href="' . esc_url( lddfw_drivers_page_url( 'lddfw_screen=order&lddfw_orderid=' . $orderid ) ) . '">' . $custom_fields . '</a>';
						}
					}
				}
	...
	public function lddfw_assign_to_driver( $driver_id ) {
		...
		if ( ! empty ( $results ) ) {
			...
			foreach ( $results as $result ) {
				...
				if ( lddfw_fs()->is__premium_only() ) {
					if ( lddfw_fs()->is_plan( 'premium', true ) ) {
						$order_number_html = '<a class="btn lddfw_order_view btn-secondary btn-sm" href="' . esc_url( lddfw_drivers_page_url( 'lddfw_screen=order&lddfw_orderid=' . $orderid ) ) . '">' . esc_html( __( 'Order #', 'lddfw' ) ) . $orderid . '</a>';
					}
				}
	 ...
	 * @return int
	 */
	public function lddfw_claim_orders_counts__premium_only() {
