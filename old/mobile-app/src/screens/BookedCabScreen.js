...
export default function BookedCabScreen(props) {
    ...
    const goBack = () => {
        props.navigation.navigate('TabRoot', { screen: 'RideList' });
    }
                            ...
                            <ScrollView showsVerticalScrollIndicator={false} style={{ flex:1}}>
                                {Object.keys(curBooking.driverOffers).map(key=>  
                                    <View key={key} style={{height: 125, marginBottom: 20,borderColor: 'black', borderWidth: 2,borderRadius: 10,}}>
                                        <View style={{marginTop:8, marginBottom:8, flexDirection: 'row',width:'100%',justifyContent:'space-evenly',padding:2,height:50}} >
                                            <View style={{flexDirection: 'row',width:'68%',height:50}}>
                                                <Image source={curBooking && curBooking.driverOffers[key].driver_image ? { uri: curBooking.driverOffers[key].driver_image} : require('../../assets/images/profilePic.png')} style={{ borderRadius: 50 / 2, width: '25%', height: 50 }} />
                                        ...
                                        <Text style={{ color: colors.BLACK, fontSize: 16,fontWeight: '600',marginTop:10,marginLeft:65}}>{moment(curBooking.driverOffers[key].deliveryDate).format('lll')}</Text>
                                        <View style={{flexDirection:'row',marginLeft:65}}>
                                        <Text style={{ color: colors.BLACK, fontSize: 12,marginTop:3}}>{t('driver_distance')} - </Text>
                                        <Text style={{ color: colors.BLACK, fontSize: 16, fontWeight: '600', }}>{curBooking && curBooking.driverEstimates[key].timein_text ? curBooking.driverEstimates[key].timein_text : t('within_min')}</Text>
                                        </View>
                                    </View>
                                )}
                            </ScrollView>
    ...
    return (
        <View style={styles.mainContainer}>
            ...
            <OtpModal
                ...
                onMatch={(value) => value ? checkCat(1) ? startBooking() : endBooking() : null}
            />
        </View>
    );
