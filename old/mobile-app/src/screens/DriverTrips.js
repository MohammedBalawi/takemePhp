...
import { Text, View, StyleSheet, Dimensions, FlatList, Modal, TouchableHighlight, Switch, Image, Platform } from 'react-native';
...
export default function DriverTrips(props) {
    ...
    const onPressAccept = (item) => {
        ...
        if (!settings.negativeBalance && wallet_balance <= 0) {
            if (checkCat(2) && item.prepaid && item.payment_mode == 'card') {
        ...
        else if (!settings.negativeBalance && wallet_balance > 0 && wallet_balance < item.convenience_fees && !checkCat(3)) {
            if (checkCat(2) && item.prepaid && item.payment_mode == 'card') {
                dispatch(acceptTask(item));
                setSelectedItem(null);
                setModalVisible(null);
            ...
            <FlatList
                ...
                ListEmptyComponent={
                    <View style={{ height: height, width: width }}>
                        ...
                        {!checks.carExists || !checks.carApproved || !checks.licenseAvailable || !checks.approved || !checks.driverActiveStatus ?
                            <View style={{
                                ...
                                height: 10 + (!checks.driverActiveStatus ? 70 : 0) + (!checks.carExists ? 70 : 0) + (!checks.carApproved ? 70 : 0) + (!checks.approved ? 70 : 0) + (!checks.licenseAvailable ? 70 : 0)
                            }}>
                                {!checks.carExists ?
                                    <View style={{ width: width - 40, height: 60, marginTop: 10, padding: 10, flexDirection: 'row', borderWidth: 1, borderColor: colors.BORDER_BACKGROUND, borderRadius: 8, justifyContent: 'space-between', alignItems: 'center' }}>
                                    ...
                                    : null}
                                {!checks.carApproved ?
                                    <View style={{ width: width - 40, height: 60, marginTop: 10, padding: 10, flexDirection: 'row', borderWidth: 1, borderColor: colors.BORDER_BACKGROUND, borderRadius: 5, justifyContent: 'flex-start', alignItems: 'center' }}>
                                        <View style={{ flexDirection: 'row', justifyContent: 'flex-start', alignItems: 'center' }}>
                                ...
                                {!checks.licenseAvailable ?
                                    <View style={{ width: width - 40, height: 60, marginTop: 10, padding: 10, flexDirection: 'row', borderWidth: 1, borderColor: colors.BORDER_BACKGROUND, borderRadius: 5, justifyContent: 'space-between', alignItems: 'center' }}>
                                        <View style={{ flexDirection: 'row', justifyContent: 'flex-start', alignItems: 'center' }}>
                                            <Icon name="alert-circle" type="ionicon" color={colors.RED} size={16} />
