...
export default function PaymentDetails(props) {
  ...
  const useWallet = () => {
    ...
    if (res) {
      if (parseFloat(profile.walletBalance) >= parseFloat(payDetails.payableAmount)) {
        let data = { ...payDetails };
        ...
        let data = { ...payDetails };
        data.usedWalletMoney = parseFloat(profile.walletBalance);
        data.payableAmount = (parseFloat(data.payableAmount) - parseFloat(profile.walletBalance)).toFixed(settings.decimal);
        setPayDetails(data);
      }
    } else {
      let data = { ...payDetails };
  ...
  const doPayment = (payment_mode) => {
    ...
    if (payment_mode == 'cash' || payment_mode == 'wallet') {
        ...
        if(parseFloat(payDetails.payableAmount) == 0 || curBooking.status === 'ACCEPTED'){
        ...
        if(payDetails.usedWalletMoney>0 && (booking.status == "PAYMENT_PENDING" ||  booking.status == "PENDING" || booking.status == 'NEW')){
          let walletBal = parseFloat(profile.walletBalance) - parseFloat(payDetails.usedWalletMoney);
          let tDate = new Date();
          let details = {
            type: 'Debit',
    ...
    <View style={styles.mainView}>
      <ScrollView showsVerticalScrollIndicator={false} style={styles.scrollStyle}>
        <View style={{ flex: 1, flexDirection: 'column' }}>
          ...
          {profile ?
            <View style={{ flex: 1, flexDirection: isRTL?'row-reverse':'row', justifyContent: 'space-between', paddingLeft: 25, paddingRight: 25 }}>
              <Text style={{ color: colors.BLACK, textAlign: isRTL?'right':'left', lineHeight: 45, fontSize: 16 }}>{profile.usertype == 'customer' ? t('your_fare'): t('total_fare')}</Text>
          ...
          {profile && profile.usertype == 'customer'  && (booking.status == 'NEW' || booking.status == 'PAYMENT_PENDING' ||  booking.status == "PENDING")? 
            <View style={{ flex: 1 }}>
              <CheckBox
                center
                disabled={parseFloat(profile.walletBalance) > 0 ? false : true}
                title={settings.swipe_symbol===false? t('use_wallet_balance') + settings.symbol + (parseFloat(profile.walletBalance) - parseFloat(payDetails.usedWalletMoney)).toFixed(settings.decimal) + ')'
                  : t('use_wallet_balance') + (parseFloat(profile.walletBalance) - parseFloat(payDetails.usedWalletMoney)).toFixed(settings.decimal) + settings.symbol + ')'}
        ...
        {payDetails.payableAmount == 0 ?
          <View style={styles.buttonContainer}>
            <TouchableOpacity
              ...
              <Text style={styles.buttonTitle}>{profile && profile.usertype == 'customer'?t('paynow_button'): t('ok')}</Text>
            </TouchableOpacity>
          </View>
          :
        ...
        {profile && profile.usertype == 'customer' && booking.status =='PAYMENT_PENDING'?
          <TouchableOpacity onPress={cancelCurBooking} style={styles.vew2}>
