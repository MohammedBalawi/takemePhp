...
export default function CarEditScreen(props) {
    ...
    useEffect(() => {
        if (cartypes) {
            ...
            for (let i = 0; i < cartypes.length; i++) {
                arr.push({ label: cartypes[i].name, value: cartypes[i].name });
            }
            if (arr.length > 0) {
    ...
    const onSave = () => {
        if (state.vehicleNumber && state.vehicleNumber.length > 1 && state.vehicleMake && state.vehicleMake.length > 1 && state.vehicleModel && state.vehicleModel.length > 1) {
            if (blob) {
                setLoading(true);
    ...
    return (
        <View style={styles.mainView}>
            <View style={{ height: '86%', marginTop: -2, backgroundColor: MAIN_COLOR }}>
                                    ...
                                    <View style={styles.buttonContainer}>
                                        {!car ?
                                            <Button
                                                ...
                                                buttonStyle={[styles.registerButton, { marginTop: state.usertype == 'driver' ? 30 : 10, backgroundColor: MAIN_COLOR }]}
                                            />
                                            : null}
